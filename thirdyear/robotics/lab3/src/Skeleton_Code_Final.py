...
class colourIdentifier():
	...
	def callback(self, data):
		...
		if len(cnts) > 0:
			if self.green_flag == True and (self.red_flag == False and self.blue_flag == False):
				c = max(cnts, key=cv2.contourArea)
				M = cv2.moments(c)
				...
				if M['m00'] > 0:
					...
					desired_velocity.angular.z = -float(err) / 100
					pub.publish(desired_velocity) # publisher is gazebo
					rate.sleep()
		...
		# if self.green_flag == True and (self.red_flag == False or self.blue_flag == False):
		# 	pub = rospy.Publisher('mobile_base/commands/velocity', Twist)
		# 	rate = rospy.Rate(10) #10h
		# 	desired_velocity = Twist()
		# 	desired_velocity.linear.x = 0.2 # Forward with 0.2 m/sec.
...
def main(args):
	...
	rospy.init_node('Skeleton_Code_Final', anonymous=True)
	cI = colourIdentifier()
	rospy.spin()
